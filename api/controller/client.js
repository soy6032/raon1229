...
exports.getTemplate = async (req, res, next) => {
    resObj = obj_init(resObj);
    const query = `SELECT t.idx, t.name, t.contents, t.provider, IF(t.price = '0', '무료', t.price) as price, ti.img_path FROM box.template as t join box.template_img as ti on t.idx = ti.t_group WHERE ti.order = 1 ORDER BY FIELD(t.idx, 10,1,2,3,4,5,6,7,8) asc`;
...
// exports.getTemplate = async (req, res, next) => {
//     resObj = obj_init(resObj);
//     const query = `SELECT t.idx, t.name, t.contents, t.provider, IF(t.price = '0', '무료', t.price) as price, ti.img_path FROM makeredu.template as t join makeredu.template_img as ti on t.idx = ti.t_group WHERE ti.order = 1 ORDER BY FIELD(t.idx, 10,1,2,3,4,5,6,7,8) asc`;
//     const result = await db.conn(query);
//     if(!result) {
//         resObj.msg = "템플릿 가져오기 실패하였습니다.";
//         resObj.state = false;
